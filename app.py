...
# ---- Demo mode ----
from utils.demo_mode import is_demo, demo_banner
...
def sidebar_ynab_settings():
    """
    In demo mode YNAB is hidden entirely — no token input is shown and
    all return values are None / False so callers behave as if YNAB is
...
def merchant_editor(df: pd.DataFrame):
    ...
    if is_demo():
        st.info(
            "🎭 Merchant category editing is disabled in demo mode — "
            "changes would not persist on the public deployment."
        )
        return
    ...
    for merchant, spent in merchants.items():
        ...
        if current_cat is None:
            cats_for_merchant = (
                df[df["MerchantClean"] == merchant_clean]["Category"].value_counts()
            )
...
# ===============================
def main():
    # Show demo banner before anything else
    if is_demo():
        demo_banner()
    ...
    with tabs[2]:
        ...
        st.markdown(
            ...
            "transactions that you know won't recur."
        )
        ...
        exclude_cats = st.multiselect("Exclude categories from forecast", all_cats, default=[])
        ...
        if not candidates.empty:
            ...
            for idx, row in candidates.iterrows():
                date_str = row["Date"].date().isoformat() if "Date" in row and pd.notna(row["Date"]) else "Unknown date"
                desc = str(row.get("Description", ""))[:40]
                amount = float(row.get("Paid out", 0.0))
                options[f"{date_str} | £{amount:,.2f} | {desc}"] = idx
    ...
    with tabs[6]:
        ...
        with st.expander("Configure category budget modes & amounts"):
            ...
            if st.button("💾 Save budget settings"):
                if not is_demo():
                    ...
                    st.info("Budget settings cannot be saved in demo mode.")
        ...
        if bv.empty:
            ...
            st.bar_chart(bv.set_index("Category")[["Budget", "Actual"]])
    ...
    with tabs[8]:
        ...
        if is_demo():
            st.info(
                "🎭 YNAB integration is not available in demo mode. "
                "Connect your own YNAB account when running the app locally."
            )
        elif not token or not budget_id:
